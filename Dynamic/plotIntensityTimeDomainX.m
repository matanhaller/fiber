...
for i=1:numel(N)
    ...
    disp(n);
    if i == 1
        EzStructs = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/Ez_n=%d.mat', n));
        EphiStructs = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/Ephi_n=%d.mat', n));
        ErhoStructs = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/Erho_n=%d.mat', n));
        eta0HzStructs = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/eta0Hz_n=%d.mat', n));
        eta0HphiStructs = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/eta0Hphi_n=%d.mat', n));
        eta0HrhoStructs = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/eta0Hrho_n=%d.mat', n));
    else
        EzStructs(i) = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/Ez_n=%d.mat', n));
        EphiStructs(i) = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/Ephi_n=%d.mat', n));
        ErhoStructs(i) = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/Erho_n=%d.mat', n));
        eta0HzStructs(i) = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/eta0Hz_n=%d.mat', n));
        eta0HphiStructs(i) = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/eta0Hphi_n=%d.mat', n));
        eta0HrhoStructs(i) = load(sprintf('Time Domain/epsilon=2,beta=0.8,hi/eta0Hrho_n=%d.mat', n));
    end
end
...
for t=95:105
    ...
    for i=1:numel(N)
        ...
        EzInv(:,0.5*numel(y)+1:end) = EzInv(:,0.5*numel(y)+1:end) + 1j*EzTotal;
        EzInv(:,1:0.5*numel(y)) = EzInv(:,1:0.5*numel(y)) + (-1j)^n*flip(EzTotal, 2);
        
        EphiInv(:,0.5*numel(y)+1:end) = EphiInv(:,0.5*numel(y)+1:end) + 1j*EphiTotal;
        EphiInv(:,1:0.5*numel(y)) = EphiInv(:,1:0.5*numel(y)) + (-1j)^n*flip(EphiTotal, 2);
        
        ErhoInv(:,0.5*numel(y)+1:end) = ErhoInv(:,0.5*numel(y)+1:end) + 1j*ErhoTotal;
        ErhoInv(:,1:0.5*numel(y)) = ErhoInv(:,1:0.5*numel(y)) + (-1j)^n*flip(ErhoTotal, 2);
        
        eta0HzInv(:,0.5*numel(y)+1:end) = eta0HzInv(:,0.5*numel(y)+1:end) + 1j*eta0HzTotal;
        eta0HzInv(:,1:0.5*numel(y)) = eta0HzInv(:,1:0.5*numel(y)) + (-1j)^n*flip(eta0HzTotal, 2);
        
        eta0HphiInv(:,0.5*numel(y)+1:end) = eta0HphiInv(:,0.5*numel(y)+1:end) + 1j*eta0HphiTotal;
        eta0HphiInv(:,1:0.5*numel(y)) = eta0HphiInv(:,1:0.5*numel(y)) + (-1j)^n*flip(eta0HphiTotal, 2);
        
        eta0HrhoInv(:,0.5*numel(y)+1:end) = eta0HrhoInv(:,0.5*numel(y)+1:end) + 1j*eta0HrhoTotal;
